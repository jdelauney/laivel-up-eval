...
Les mesures au clavier de `qa/README.md` (« Point 1 ») restaient vraies en position — `s1` et `s3` avaient le cadrage en tête, `s2` le marché — mais décrivaient l'ancien sens. Elles sont désormais inversées : `s1` et `s3` ont le marché en tête, `s2` le cadrage. Une reprise de la tournée navigateur (captures et comptage réel des arrêts de tabulation) reste à faire pour reproduire les mesures dans le nouveau sens — non couvert par ce lot, faute d'outillage navigateur disponible ici ; consigné comme point non levé.
