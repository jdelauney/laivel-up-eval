...
## La tournée est rejouée une nouvelle fois, tour 5 (31/08) — après `ec18346`
...
## La tournée est rejouée une nouvelle fois, tour 6 (31/08) — après `61f9576`

`61f9576` (« isolate hint-budget's floor and framing-reference refusals ») réécrit trois `verification` de cause — celles que le plancher de deux causes laisse debout, `s1-c-cdn`, `s2-c-double`, `s3-c-cache` — pour citer la ligne de rapport qui porte l'inférence plutôt qu'un indice supprimé au tour 3, et ajoute un `describe('the two-cause floor', …)` qui isole les trois refus des tours 4 et 5. Aucun `framings[].text` ni `hints[].label` ne bouge, et `verification` n'est rendue qu'à la révélation d'une situation : seule cette surface pouvait bouger. Rejouée en entier, mêmes conditions (Playwright, deux gabarits, page pleine, `scrollY = 0` vérifié avant chaque lecture de position, aucune mesure recopiée) : détail complet, `qa/README.md`, réécrit de zéro.

Ce qui restait vrai sans remesure l'est resté : ordre DOM des deux panneaux, alternance, arrêts de tabulation (5 puis 6), parité desktop (écart de sommet nul), sommets de panneau mobiles et dépassement de l'action de trancher (700px/957px à l'ouverture, 622px/819px après cinq achats, deltas −78px/−138px) tiennent à l'identique sur les trois situations — sept des neuf états capturés par gabarit sont inchangés en dimensions.

Ce qui bouge est limité aux deux états de révélation joués par cette tournée : `s1` (1440×1920 → **1440×1998** desktop, +78px ; 390×2610 → **390×2649** mobile, +39px) et `s2` (1440×1736 → **1440×1834** desktop, +98px ; 390×2434 → **390×2493** mobile, +59px), chacun expliqué par l'allongement d'une seule `verification` désormais plus longue de citer le fait du rapport plutôt qu'un indice absent. `s3` n'est jamais révélée dans cette tournée (comme aux tournées précédentes), donc sa `verification` réécrite (`s3-c-cache`) n'a aucune capture qui la montre.

**Un gap de la revue précédente fermé au passage** : le point 6 de `qa/README.md` (tour 5) affirmait « trois suppositions réécrites » sur le panneau de cadrage de `s1` ; recompté ici, ce sont deux (`s1-f3`, `s1-f4`), la troisième (`s1-f5`) garde son texte d'origine depuis le tour 4 — corrigé dans la version courante du fichier.
