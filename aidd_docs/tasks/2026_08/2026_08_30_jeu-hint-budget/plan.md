...
| Trois critères, pesés **2 · 1 · 1**, tous sur la seule dimension `pilotage-contexte` | Le mapping `harness` du placeholder disparaît, comme il a disparu de `g1-2` et `g1-3` : les six premiers groupes portent la signature, seul le septième porte les axes du référentiel officiel. `c1` (frugalité) pèse 2, `c2` (ordre) et `c3` (fondement) pèsent 1 chacun — l'équilibre initial entre « trancher frugalement » et « cadrer » tient toujours, poids 2 contre 1+1 |
| **Correction du 30/08, après revue.** `c2` était initialement un critère unique (poids 2) exigeant à la fois l'ordre et le fondement du cadrage, sous une question affichée qui ne parlait que d'ordre (« Le contexte a-t-il été posé avant le premier indice ? »). Un joueur qui cadrait exactement en premier, mais de façon incomplète, lisait « manqué » sur un critère que sa question ne laissait pas deviner — un décalage entre le contrat affiché et la règle réellement appliquée. Décision produit : **`c2` est scindé en deux** — `c2` ne lit plus que l'ordre (`framed-first-at-least`, nouvelle règle déclarative), `c3` reprend le fondement seul (`grounded-framings-at-least`, conservée mais recentrée sur `SituationReading.framingGrounded`, sans plus jamais lire `framedFirst`) | Le décalage contrat/code prime sur la simplicité d'un seul critère combiné. Deux règles à un seul axe de lecture chacune, plutôt qu'une règle à deux axes cachés derrière une question qui n'en montre qu'un |
