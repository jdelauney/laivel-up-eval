...
| **Facade** | `game-session.facade.ts` | Seul point d'entrée de l'UI : `start()`, `submitAnswer()`, `nextGame()`, `getProgress()`, `getVerdict()`, `auditTrail()`, `resume()`, `resetSession()`. Cache registry, evaluators, scoring, persistence, horloge. L'export reste à écrire. |
