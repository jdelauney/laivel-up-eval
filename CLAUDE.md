...
Before planning, writing, or reviewing **any** code, load and follow these project skills. They encode the team's way of building and are non-negotiable defaults; deviate only with an explicit, stated reason.

- `.claude/skills/user-clean-code-typescript` — clean-code TypeScript/React/Next.js conventions (naming, limits, lint do/don't, security, anti-patterns). Also auto-loads on `*.ts`/`*.tsx` via its `paths:`.
...
## CONSTRAINTS
- AVOID TO USE SCRIPTS PYTHON, POWERSHELL, BASH, HEREDOC FOR READ, CREATE OR EDIT FILES. ALWAYS USE INTERNAL TOOLS.